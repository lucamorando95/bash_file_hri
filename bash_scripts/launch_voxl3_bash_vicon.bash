...
tmux new-session -d -s $session -n home
  ...
  read -p "Continuing in 1.5 seconds before launch Vicon Node.." -t 0.5
   ...
   tmux send-keys -t $session:1.1 'roslaunch mocap_vicon vicon.launch' C-m
   ...
   tmux send-keys -t $session:1.2 'rosrun master_sync_fkie master_sync' C-m
 ...
 #Launch the admittance control tele_control_voxl_sim.launch
   ...
   tmux send-keys -t $session:1.3 'rosrun master_discovery_fkie master_discovery' C-m
